...
class GCRInstanceCostVisualizer:
    def __init__(self):
    ...
    def plot_cost_comparison(self, results_df, series_type="B", save_path=None):
        """Create comprehensive cost comparison charts for instance-based billing"""
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 12))
        fig.suptitle(f'Google Cloud Run {series_type.upper()}-Series Instance-Based Cost Analysis', fontsize=16, fontweight='bold')
        ...
        # 2. Cost Breakdown (CPU vs Memory) - CHANGED: No request costs
        width = 0.35
        x = np.arange(len(results_df))
        
        bars2a = ax2.bar(x, results_df['cpu_cost'], width, label='CPU Cost', 
                color='lightblue', alpha=0.8)
        bars2b = ax2.bar(x, results_df['memory_cost'], width, bottom=results_df['cpu_cost'],
                label='Memory Cost', color='lightgreen', alpha=0.8)
        
        ax2.set_title('Cost Breakdown: CPU vs Memory')
    ...
    def plot_simple_cost_comparison(self, results_df, series_type="B", save_path=None):
        ...
        fig.suptitle(f'Google Cloud Run {series_type.upper()}-Series Instance-Based Simple Cost Analysis', fontsize=16, fontweight='bold')
        ...
        # 2. Cost Breakdown (CPU vs Memory)
        x2 = np.arange(len(results_df))
        width = 0.7
        
        bars2a = ax2.bar(x2, results_df['cpu_cost'], width, label='CPU Cost', 
                color='lightblue', alpha=0.8)
        bars2b = ax2.bar(x2, results_df['memory_cost'], width, bottom=results_df['cpu_cost'],
                label='Memory Cost', color='lightgreen', alpha=0.8)
        
        ax2.set_title('Cost Breakdown: CPU vs Memory', fontsize=14, fontweight='bold')
    ...
    def plot_cost_efficiency_analysis(self, results_df, series_type="B", save_path=None):
        """Create detailed cost efficiency analysis for instance-based billing"""
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 12))
        fig.suptitle(f'{series_type.upper()}-Series Instance-Based Cost Efficiency Analysis', fontsize=16, fontweight='bold')
        
        # 1. Cost per Request (for comparison purposes)
        cost_per_request = results_df['cost_per_million_requests']
        ...
        # 2. CPU vs Memory Cost Distribution
        cpu_percentage = (results_df['cpu_cost'] / results_df['total_cost']) * 100
        memory_percentage = (results_df['memory_cost'] / results_df['total_cost']) * 100
        
        x = np.arange(len(results_df))
        width = 0.6
        
        ax2.bar(x, cpu_percentage, width, label='CPU %', color='lightblue', alpha=0.8)
        ax2.bar(x, memory_percentage, width, bottom=cpu_percentage, label='Memory %', color='lightgreen', alpha=0.8)
        ax2.set_title('Cost Distribution: CPU vs Memory')
        ax2.set_ylabel('Percentage of Total Cost')
        ax2.set_xticks(x)
        ax2.set_xticklabels(results_df['config'], rotation=45)
        ax2.legend()
        ...
        # Cost per vCPU-hour and per GiB-hour
        vcpu_hours = (results_df['avg_cpu'] / 1000) * (results_df['duration_seconds'] / 3600)
        gib_hours = (results_df['avg_memory'] / 1024) * (results_df['duration_seconds'] / 3600)
        cpu_efficiency = results_df['cpu_cost'] / vcpu_hours
        memory_efficiency = results_df['memory_cost'] / gib_hours
        
        x = np.arange(len(results_df))
        width = 0.35
        
        ax4.bar(x - width/2, cpu_efficiency, width, label='CPU $/vCPU-hr', color='lightblue', alpha=0.8)
        ax4.bar(x + width/2, memory_efficiency, width, label='Memory $/GiB-hr', color='lightgreen', alpha=0.8)
        ax4.set_title('Resource Cost Efficiency')
        ax4.set_ylabel('Cost per Resource Hour (USD)')
        ax4.set_xticks(x)
    ...
    def create_cost_summary_table(self, results_df, series_type="B", save_path=None):
        ...
        summary_data = {
            ...
            'CPU $': results_df['cpu_cost'].apply(lambda x: f"${x:.4f}"),
            'Memory $': results_df['memory_cost'].apply(lambda x: f"${x:.4f}"),
            'Total $': results_df['total_cost'].apply(lambda x: f"${x:.4f}"),
            '$/M Req': results_df['cost_per_million_requests'].apply(lambda x: f"${x:.2f}"),
        ...
        plt.title(f'{series_type.upper()}-Series Instance-Based Cost Analysis Summary', fontsize=16, fontweight='bold', pad=20)
    ...
    def generate_all_visualizations(self, calculator, data_directory, series_type="B", output_directory=None):
        ...
        if output_directory is None:
            output_directory = f"{series_type.lower()}_series_gcr_instance_costs"
        ...
        print("Generating instance-based cost visualizations...")
        
        # 1. Cost comparison charts
        self.plot_cost_comparison(
            results_df, 
            series_type,
            save_path=os.path.join(output_directory, "instance_cost_comparison.png")
        )
        ...
        self.plot_cost_efficiency_analysis(
            results_df,
            series_type,
            save_path=os.path.join(output_directory, "instance_cost_efficiency.png")
        )
        ...
        summary_df = self.create_cost_summary_table(
            results_df,
            series_type,
            save_path=os.path.join(output_directory, "instance_summary_table.png")
        )
        ...
        # Save results to CSV
        series_prefix = f"{series_type.lower()}_series_instance"
        ...
        print(f"All instance-based visualizations saved to {output_directory}/")
...
if __name__ == "__main__":
    # Import the instance cost calculator (assuming it's in the same directory)
    from gcr_cost_calculator_inst import GCRInstanceCostCalculator
    
    parser = argparse.ArgumentParser(description='GCR Instance-Based Cost Visualizer for A, B, and C Series')
    ...
    visualizer = GCRInstanceCostVisualizer()
    ...
    print(f"Generating instance-based visualizations for {args.series}-series configurations...")
    ...
    if results_df is not None:
        print("\nInstance-Based Visualization Summary:")
        print("====================================")
