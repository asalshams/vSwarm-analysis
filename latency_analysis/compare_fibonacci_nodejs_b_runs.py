...
RUN_DIRS: Dict[str, str] = {
    "B1": "results_fibonacci_nodejs_b1",
    "B2": "results_fibonacci_nodejs_b2",
    "B3": "results_fibonacci_nodejs_b3",
    "B4": "results_fibonacci_nodejs_b4",
    "B5": "results_fibonacci_nodejs_b5",
}

# Same capacity plan as in Python B1–B5 table
...
def read_detailed_csvs(base_dir: str) -> pd.DataFrame:
    ...
    for run, subdir in RUN_DIRS.items():
        csv_path = os.path.join(base_dir, subdir, f"fibonacci_nodejs_{run.lower()}_detailed_analysis.csv")
    ...
    if not rows:
        raise FileNotFoundError("No detailed analysis CSVs found for Node.js B1–B5")
...
def read_timeseries_data(base_dir: str) -> Dict[str, pd.DataFrame]:
    ...
    for run, subdir in RUN_DIRS.items():
        pod_csv_path = os.path.join(base_dir, subdir, "fibonacci_nodejs_{run.lower()}_pod_monitoring.csv")
...
def ensure_output_dir(base_dir: str) -> str:
    out_dir = os.path.join(base_dir, "charts_fibonacci_nodejs_b_comparison")
    os.makedirs(out_dir, exist_ok=True)
    return out_dir


def save_aggregated_csv(grouped: pd.DataFrame, out_dir: str) -> None:
    out_csv = os.path.join(out_dir, "fibonacci_nodejs_b_aggregated.csv")
...
def plot_efficiency(df_agg: pd.DataFrame, out_dir: str) -> None:
    ...
    for run_label, sub in df_agg.groupby("run_label"):
        plt.plot(sub["target_rps"], sub["throughput_rps_mean"], marker="o", label=run_label)
    ...
    plt.title("Achieved throughput vs Target RPS (Fibonacci Node.js B1–B5)")
...
def plot_latency_vs_target(df_agg: pd.DataFrame, out_dir: str) -> None:
    ...
    for col, label in metrics:
        ...
        for run_label, sub in df_agg.groupby("run_label"):
            ...
            plt.fill_between(sub["target_rps"], y - yerr, y + yerr, alpha=0.12)
        plt.title(f"{label} latency vs Target RPS (Fibonacci Node.js B1–B5)")
        plt.xlabel("Target RPS")
...
def plot_latency_vs_throughput(df_agg: pd.DataFrame, out_dir: str) -> None:
    ...
    for col, label in metrics:
        ...
        sns.set_style("whitegrid")
        for run_label, sub in df_agg.groupby("run_label"):
            plt.plot(sub["throughput_rps_mean"], sub[f"{col}"], marker="o", label=run_label)
            y = sub[f"{col}"].values
            yerr = sub[f"{col.replace('_mean', '_std')}"].fillna(0).values
            plt.fill_between(sub["throughput_rps_mean"], y - yerr, y + yerr, alpha=0.12)
        plt.title(f"{label} latency vs Achieved Throughput (Fibonacci Node.js B1–B5)")
...
def plot_heatmap(df_agg: pd.DataFrame, out_dir: str, value_col: str, title: str, filename: str) -> None:
    pivot = df_agg.pivot_table(index="run", columns="target_rps", values=value_col, aggfunc="mean")
...
def plot_timeseries_comparison(timeseries_data: Dict[str, pd.DataFrame], out_dir: str) -> None:
    ...
    plt.title("CPU Usage Comparison Over Time (Fibonacci Node.js B1–B5)")
    ...
    plt.title("Memory Usage Comparison Over Time (Fibonacci Node.js B1–B5)")
    plt.xlabel("Test Progress (%)")
    ...
    plt.title("Pod Count Comparison Over Time (Fibonacci Node.js B1–B5)")
    plt.xlabel("Test Progress (%)")
...
def plot_resource_efficiency(df_agg: pd.DataFrame, timeseries_data: Dict[str, pd.DataFrame], out_dir: str) -> None:
    ...
    plt.title("CPU Efficiency vs Throughput (Fibonacci Node.js B1–B5)")
    plt.legend(loc="best", fontsize=10)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    plt.ylabel("Average Throughput (RPS)")
    plt.title("Memory Efficiency vs Throughput (Fibonacci Node.js B1–B5)")
    plt.legend(loc="best", fontsize=10)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
