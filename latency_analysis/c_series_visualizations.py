...
RUN_CONFIG = {
    ...
    "C4": {"pods": "7", "container_cpu": "250m", "container_mem": "250Mi", "target_conc": "25"},
    "C5": {"pods": "8", "container_cpu": "200m", "container_mem": "200Mi", "target_conc": "20"},
}
...
class CSeriesVisualizer:
    ...
    def create_scaling_efficiency_comparison(self, max_data: pd.DataFrame):
        ...
        config_text += "C4: 7 pods × 250m CPU = 1.75 vCPU total (Conc=25)\n"
        config_text += "C5: 8 pods × 200m CPU = 1.6 vCPU total (Conc=20)"
    ...
    def _create_latency_vs_target_charts(self, runtime: str, grouped_data: pd.DataFrame, runtime_dir: str, runtime_label: str):
        ...
        for metric_col, metric_label in metrics:
            ...
            ax.set_title(f'{runtime_label} C-Series: {metric_label} Latency vs Target RPS\nMulti-Pod Scaling Analysis', 
                        fontsize=14, fontweight='bold', pad=20)
            ax.set_ylim(0, 500)  # Set y-axis limit to 500ms
    ...
    def _create_aggregated_csv(self, runtime: str, runtime_data: pd.DataFrame, runtime_dir: str, runtime_label: str):
        ...
        try:
            ...
            with open(output_path, 'w') as f:
                ...
                f.write("# for each configuration (C1-C5) and target RPS level.\n")
                f.write("# \n")
                f.write("# Columns Description:\n")
                f.write("# - run: Configuration (C1, C2, C3, C4, C5)\n")
                ...
                f.write("# \n")
                f.write("# C-Series Configuration (Constant Total Resources):\n")
                f.write("# - C1: 1 pod, 2000m CPU, 2Gi memory, concurrency=2000\n")
                f.write("# - C2: 2 pods, 1000m CPU each, 1Gi memory each, concurrency=1000\n")
                f.write("# - C3: 4 pods, 500m CPU each, 500Mi memory each, concurrency=500\n")
                f.write("# - C4: 7 pods, 250m CPU each, 250Mi memory each, concurrency=250\n")
                f.write("# - C5: 8 pods, 200m CPU each, 200Mi memory each, concurrency=200\n")
                f.write("# \n")
                f.write("# Total resources: C1-C3 maintain 2000m CPU, 2Gi memory; C4-C5 use fewer total resources\n")
                f.write("# Tests horizontal scaling efficiency with higher resource allocation.\n")
                f.write("# \n")
            ...
            if summary_data:
                ...
                with open(summary_path, 'w') as f:
                    ...
                    f.write("# \n")
                    f.write("# Configuration: C1 (1 pod, 2000m CPU) to C5 (8 pods, 200m CPU each)\n")
                    f.write("# C1-C3 maintain constant total resources (2000m CPU, 2Gi memory); C4-C5 use fewer total resources\n")
    ...
    # =========================================================================
    # COMPREHENSIVE RESOURCE SCALING ANALYSIS (LIKE A-SERIES AND B-SERIES)
    # =========================================================================
    ...
    def _create_configuration_aggregated_analysis(self, all_runtime_data: Dict[str, pd.DataFrame], resource_data: Dict[str, Dict[str, pd.DataFrame]]):
        ...
        # Create charts for each configuration (C1, C2, C3, C4, C5)
        for config in ["C1", "C2", "C3", "C4", "C5"]:
            # Create config-specific directory
            config_dir = os.path.join(self.output_dir, f"c{config[1]}")
    ...
    def _create_config_latency_comparison(self, config: str, all_runtime_data: Dict[str, pd.DataFrame], config_dir: str):
        ...
        for metric_col, metric_label in metrics:
            ...
            ax.grid(True, alpha=0.3)
            ax.set_yscale('log')
            ax.set_ylim(0.1, 500)  # Set y-axis limit to 500ms (log scale)
    ...
    def _create_cpu_timeseries_comparison(self, resource_data: Dict[str, Dict[str, pd.DataFrame]]):
        ...
        fig, axes = plt.subplots(3, 1, figsize=(16, 12), sharex=True)
        fig.suptitle('C-Series CPU Usage Timeseries Comparison (Aggregated)\nMulti-Pod Scaling Analysis Across All Runtimes and Configurations', 
                     fontsize=16, fontweight='bold')
            ...
            if runtime in resource_data:
                ...
                for run in ["C1", "C2", "C3", "C4", "C5"]:
                    if run in resource_data[runtime]:
                        ...
                        if 'cpu_usage_millicores' in data.columns and 'timestamp' in data.columns:
                            config = RUN_CONFIG[run]
                            label = f"{run} | {config['pods']}×{config['container_cpu']}"
        ...
        filename = "c_series_cpu_timeseries_comparison.png"
        output_path = os.path.join(self.output_dir, filename)
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
        ...
        fig, axes = plt.subplots(3, 1, figsize=(16, 12), sharex=True)
        fig.suptitle('C-Series CPU Usage per Pod Timeseries Comparison\nMulti-Pod Scaling Analysis Across All Runtimes and Configurations', 
                     fontsize=16, fontweight='bold')
        ...
        filename = "c_series_cpu_per_pod_timeseries_comparison.png"
        output_path = os.path.join(self.output_dir, filename)
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_memory_timeseries_comparison(self, resource_data: Dict[str, Dict[str, pd.DataFrame]]):
        ...
        # Create aggregated version
        fig, axes = plt.subplots(3, 1, figsize=(16, 12), sharex=True)
        fig.suptitle('C-Series Memory Usage Timeseries Comparison (Aggregated)\nMulti-Pod Scaling Analysis Across All Runtimes and Configurations', 
                     fontsize=16, fontweight='bold')
            ...
            if runtime in resource_data:
                ...
                for run in ["C1", "C2", "C3", "C4", "C5"]:
                    if run in resource_data[runtime]:
                        ...
                        if 'memory_usage_mib' in data.columns and 'timestamp' in data.columns:
                            config = RUN_CONFIG[run]
                            label = f"{run} | {config['pods']}×{config['container_mem']}"
        ...
        filename = "c_series_memory_timeseries_comparison.png"
        output_path = os.path.join(self.output_dir, filename)
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
        ...
        # Create per-pod version
        fig, axes = plt.subplots(3, 1, figsize=(16, 12), sharex=True)
        fig.suptitle('C-Series Memory Usage per Pod Timeseries Comparison\nMulti-Pod Scaling Analysis Across All Runtimes and Configurations', 
                     fontsize=16, fontweight='bold')
        ...
        filename = "c_series_memory_per_pod_timeseries_comparison.png"
        output_path = os.path.join(self.output_dir, filename)
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_pod_count_timeseries_comparison(self, resource_data: Dict[str, Dict[str, pd.DataFrame]]):
        ...
        fig, axes = plt.subplots(3, 1, figsize=(16, 12), sharex=True)
        fig.suptitle('C-Series Pod Count Timeseries Comparison\nMulti-Pod Scaling Analysis Across All Runtimes and Configurations', 
                     fontsize=16, fontweight='bold')
        ...
        filename = "c_series_pod_count_timeseries_comparison.png"
        output_path = os.path.join(self.output_dir, filename)
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_cpu_scaling_analysis(self, resource_data: Dict[str, Dict[str, pd.DataFrame]]):
        ...
        for runtime in RUNTIME_ORDER:
            if runtime in resource_data:
                ...
                if configs:
                    config_indices = [1, 2, 3, 4, 5]  # C1=1, C2=2, etc.
        ...
        ax1.set_xlabel('Configuration (C1 → C5)', fontsize=12, fontweight='bold')
        ax1.set_ylabel('Total CPU Usage (millicores)', fontsize=12, fontweight='bold')
        ax1.set_title('C-Series CPU Scaling Analysis\nTotal CPU Usage Across Multi-Pod Configurations', 
                     fontsize=14, fontweight='bold', pad=20)
        ...
        # Add configuration details
        config_text = "Multi-Pod Configuration:\n"
        config_text += "C1: 1 pod × 2000m CPU\n"
        config_text += "C2: 2 pods × 1000m CPU each\n"
        config_text += "C3: 4 pods × 500m CPU each\n"
        config_text += "C4: 7 pods × 250m CPU each\n"
        config_text += "C5: 8 pods × 200m CPU each"
        ...
        ax2.set_xlabel('Configuration (C1 → C5)', fontsize=12, fontweight='bold')
        ax2.set_ylabel('CPU Usage per Pod (millicores)', fontsize=12, fontweight='bold')
        ax2.set_title('C-Series CPU Efficiency Analysis\nCPU Usage per Pod Across Multi-Pod Configurations', 
                     fontsize=14, fontweight='bold', pad=20)
        ...
        theoretical_limits = [2000, 1000, 500, 250, 200]  # CPU limits per pod
        ...
        filename = "c_series_cpu_scaling_analysis.png"
        output_path = os.path.join(self.output_dir, filename)
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_memory_scaling_analysis(self, resource_data: Dict[str, Dict[str, pd.DataFrame]]):
        ...
        ax1.set_title('C-Series Memory Scaling Analysis\nTotal Memory Usage Across Multi-Pod Configurations', 
                     fontsize=14, fontweight='bold', pad=20)
        ax1.set_xticks([1, 2, 3, 4, 5])
        ax1.set_xticklabels(['C1', 'C2', 'C3', 'C4', 'C5'])
        ax1.legend(fontsize=10)
        ...
        # Add configuration details
        config_text = "Multi-Pod Configuration:\n"
        config_text += "C1: 1 pod × 2Gi Memory\n"
        config_text += "C2: 2 pods × 1Gi Memory each\n"
        config_text += "C3: 4 pods × 500Mi Memory each\n"
        config_text += "C4: 7 pods × 250Mi Memory each\n"
        config_text += "C5: 8 pods × 200Mi Memory each"
        ...
        ax2.set_xlabel('Configuration (C1 → C5)', fontsize=12, fontweight='bold')
        ax2.set_ylabel('Memory Usage per Pod (MiB)', fontsize=12, fontweight='bold')
        ax2.set_title('C-Series Memory Efficiency Analysis\nMemory Usage per Pod Across Multi-Pod Configurations', 
                     fontsize=14, fontweight='bold', pad=20)
        ax2.set_xticks([1, 2, 3, 4, 5])
        ax2.set_xticklabels(['C1', 'C2', 'C3', 'C4', 'C5'])
        ax2.legend(fontsize=10)
        ...
        # Add theoretical limits (convert to MiB)
        theoretical_limits = [2048, 1024, 512, 256, 204]  # Memory limits per pod in MiB
        ax2.plot([1, 2, 3, 4, 5], theoretical_limits, 'k--', alpha=0.5, 
        ...
        filename = "c_series_memory_scaling_analysis.png"
        output_path = os.path.join(self.output_dir, filename)
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_resource_efficiency_analysis(self, resource_data: Dict[str, Dict[str, pd.DataFrame]]):
        ...
        cpu_limits = [2000, 1000, 500, 250, 200]  # millicores per pod
        memory_limits = [2048, 1024, 512, 256, 204]  # MiB per pod
        ...
        for runtime in RUNTIME_ORDER:
            if runtime in resource_data:
                ...
                for i, run in enumerate(["C1", "C2", "C3", "C4", "C5"]):
                    if run in resource_data[runtime]:
                        data = resource_data[runtime][run]
        ...
        ax1.set_title('C-Series CPU Resource Efficiency\nActual vs Theoretical CPU Usage per Pod', 
                     fontsize=14, fontweight='bold', pad=20)
        ax1.set_xticks([1, 2, 3, 4, 5])
        ax1.set_xticklabels(['C1', 'C2', 'C3', 'C4', 'C5'])
        ax1.legend(fontsize=10)
        ...
        ax2.set_title('C-Series Memory Resource Efficiency\nActual vs Theoretical Memory Usage per Pod', 
                     fontsize=14, fontweight='bold', pad=20)
        ax2.set_xticks([1, 2, 3, 4, 5])
        ax2.set_xticklabels(['C1', 'C2', 'C3', 'C4', 'C5'])
        ax2.legend(fontsize=10)
        ...
        filename = "c_series_resource_efficiency_analysis.png"
        output_path = os.path.join(self.output_dir, filename)
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_resource_utilization_heatmap(self, resource_data: Dict[str, Dict[str, pd.DataFrame]]):
        ...
        filename = "c_series_resource_utilization_heatmap.png"
        output_path = os.path.join(self.output_dir, filename)
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_per_pod_cpu_scaling_analysis(self, resource_data: Dict[str, Dict[str, pd.DataFrame]]):
        ...
        ax.set_xlabel('Configuration (C1 → C5)', fontsize=12, fontweight='bold')
        ax.set_ylabel('CPU Usage per Pod (millicores)', fontsize=12, fontweight='bold')
        ax.set_title('C-Series Per-Pod CPU Scaling Analysis\nCPU Usage per Pod Across Multi-Pod Configurations', 
                     fontsize=14, fontweight='bold', pad=20)
        ...
        filename = "c_series_per_pod_cpu_scaling_analysis.png"
        output_path = os.path.join(self.output_dir, filename)
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_per_pod_memory_scaling_analysis(self, resource_data: Dict[str, Dict[str, pd.DataFrame]]):
        ...
        ax.set_xlabel('Configuration (C1 → C5)', fontsize=12, fontweight='bold')
        ax.set_ylabel('Memory Usage per Pod (MiB)', fontsize=12, fontweight='bold')
        ax.set_title('C-Series Per-Pod Memory Scaling Analysis\nMemory Usage per Pod Across Multi-Pod Configurations', 
                     fontsize=14, fontweight='bold', pad=20)
        ax.set_xticks([1, 2, 3, 4, 5])
        ax.set_xticklabels(['C1', 'C2', 'C3', 'C4', 'C5'])
        ax.legend(fontsize=10, ncol=2)
        ...
        filename = "c_series_per_pod_memory_scaling_analysis.png"
        output_path = os.path.join(self.output_dir, filename)
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def generate_all_c_series_visualizations(self):
        ...
        # Generate comprehensive resource scaling analysis (like A-series and B-series)
